...
\else
    \def\@tbfblock#1(#2, #3){
        \pgfmathsetmacro{\tmpfactor}{2} % lager in html
        \pgfmathsetmacro{\tmpwidth}{int(#1 * 28.45 * \tmpfactor)}
        \pgfmathsetmacro{\tmpleft}{int(#2 * 28.45 * \tmpfactor)}
        \pgfmathsetmacro{\tmptop}{int(#3 * 28.45 * \tmpfactor)}
...
\newenvironment*{internal}[5]
{
    ...
    \else
        \HCode{<h3>#1</h3>}
        % \section{#1} No becuase the bar is ugly : I must define it TODO
    \fi
...
\newcommand\tbfname[2]{
    ...
    \begin{center}
        \headerfirstnamestyle{#1} \hspace{5pt} \headerlastnamestyle{#2}
    \end{center}
    \end{tbfblock}
}
