...
% Jobs
\NewDocumentEnvironment{joblist}{O{}}{%
    \setkeys{tbfarg}{x={7.8},y={3.9},width={13.2},#1}
    \pgfmathsetmacro{\tmpwidth}{int(\mmwidth * 28.45 * \htmlfactor)}
    \pgfmathsetmacro{\tmpleft}{int(\mmx * 28.45 * \htmlfactor)}
    \pgfmathsetmacro{\tmptop}{int(\mmy * 28.45 * \htmlfactor)}
    \begin{flat_frame}{\wordwork}{\mmwidth}{\mmx}{\mmy}{
        style="
            display: inline-block;
...
% Formation
\NewDocumentEnvironment{yearlist}{O{}}{%
    \setkeys{tbfarg}{x={0.7},y={3.9},width={7.1},#1}
    % Html stuff
    \pgfmathsetmacro{\tmpwidth}{int(\mmwidth * 28.45 * \htmlfactor)}
    \pgfmathsetmacro{\tmpleft}{int(\mmx * 28.45 * \htmlfactor)}
    \pgfmathsetmacro{\tmptop}{int(\mmy * 28.45 * \htmlfactor)}
    ...
    \fi
    % Create my space
    \begin{rounded_frame}{\wordeducation}{\mmwidth}{\mmx}{\mmy}{
        style="
...
\NewDocumentEnvironment{skilllist}{O{}}{%
    \setkeys{tbfarg}{x={0.7},y={15.2},width={7.1},#1}
%\newenvironmentx{skilllist}[3][1=7.1, 2=0.7, 3=15.2]{
    \pgfmathsetmacro{\tmpwidth}{int(\mmwidth * 28.45 * \htmlfactor)}
    \pgfmathsetmacro{\tmpleft}{int(\mmx * 28.45 * \htmlfactor)}
    \pgfmathsetmacro{\tmptop}{int(\mmy * 28.45 * \htmlfactor)}
    \begin{rounded_frame}{\wordlanguage}{\mmwidth}{\mmx}{\mmy}{
        style="
